...
Tests mirror source one to one: `src/morrigan/<file>.py` is exercised by `tests/test_<file>.py`. Two documented exceptions exist: the cross-cutting ensemble statistics (`tests/test_ensemble_statistics.py`) and the optional-dependency companion of the mass-loss tests (`tests/test_mass_loss_properties.py`). Every test on a physics routine asserts at least one physical invariant (conservation, positivity or boundedness, monotonicity or symmetry, or a pinned value with a discrimination guard), every test carries a docstring naming the scenario it verifies, and any test that triggers a random draw seeds the global numpy state explicitly and states the seed. Optional dependencies are import-or-skipped at module top.
