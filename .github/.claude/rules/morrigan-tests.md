...
- **`@pytest.mark.reference_pinned`**: the test pins behavior against a **published benchmark** (Kimura et al. 2025 statistics; Kegerreis et al. 2020 closed form), an **analytical limit** (Kepler's third law, the mutual-Hill-radius closed form, Laplace-Lagrange two-planet eigenfrequencies), or a **cross-implementation cross-check** (the equal-density identity between this repo's loss law and `zephyrus.collision.mass_loss`).
...
| **Mass assigned into a fraction array** | An absolute atmosphere mass in kg written into `atm_mass_fraction` | Both are floats; no bounds check on the array | Boundedness invariant: every fraction in `[0, 1]` after every event |
