#!/usr/bin/env python3
"""AST-based test-quality linter for the Morrigan test suite.

Enforces the rules in `.github/.claude/rules/morrigan-tests.md` (sections 1 + 7):
...
- Forbidden weak assertions when they stand alone as the sole meaningful
  ...
  three-class discrimination guard from morrigan-tests.md section 2 uses
...
- Optional dependencies (``hypothesis``, ``zephyrus``) imported at module top
  without a preceding ``pytest.importorskip('<name>')`` (the ``pip install
  --no-deps`` CI image otherwise fails collection).
...
SRC_DIR = REPO_ROOT / 'src' / 'morrigan'
...
# Source rule: morrigan-tests.md section 5.
OPTIONAL_DEPS = {
    'hypothesis',
    'zephyrus',
}
...
# ``tests/test_<source>.py`` (the rule in morrigan-tests.md section 12).
...
def reference_pinned_status() -> list[str]:
    ...
    for source in sorted(PHYSICS_SOURCES):
        # source is e.g. 'chemistry.py' -> companion is tests/test_chemistry.py
...
def cmd_baseline() -> int:
    ...
    allow = os.environ.get('MORRIGAN_TEST_QUALITY_ALLOW_REGRESS') == '1'
    if old and new_total > old_total and not allow:
        print(
            ...
            f'pre-existing violations), set MORRIGAN_TEST_QUALITY_ALLOW_REGRESS=1.',
            file=sys.stderr,
        )
        return 2
