...
def plot_tracks(directory):
    ...
    for n in range(ndisk):
        full_system = ascii.read(
            directory + f'/data/full_systems/full_system_{n:02d}.csv', format='fixed_width'
        )
    ...
    for n in range(ndisk):
        ...
        if masses.max() > masses.min():
            marker_sizes = size_min + (np.sqrt(masses) - np.sqrt(masses.min())) / (
                np.sqrt(masses.max()) - np.sqrt(masses.min())
            ) * (size_max - size_min)
        else:
            marker_sizes = np.full(len(masses), (size_min + size_max) / 2)

        for a, e, mp, ms, f in zip(
            remaining_system['a_AU'],
            remaining_system['ecc'],
            masses,
            marker_sizes,
            range(len(remaining_system['ecc'])),
        ):
            ...
            ax.plot(
                ...
                lw=3,
                color=c[f % len(c)],
                label=f'e = {round(e, 3)}, Mp = {mp}' + r' $M_\oplus$',
            )
        ...
        ax.plot(
            0, 0, marker='*', color='gold', markersize=15, markeredgecolor='orange', zorder=5
        )
        ax.set_aspect('equal')
        ax.set_xlabel('x (AU)')
        ax.set_ylabel('y (AU)')
...
def plot_stats(directory):
    ...
    for n in range(ndisk):
        ...
        run_mp = np.asarray(
            remaining_system['Mp'] / M_earth, dtype=float
    ...
    ax[0].errorbar(
        np.nanmean(com_run),
        np.nanmean(batch_planets),
        yerr=np.nanstd(batch_planets),
        xerr=np.nanstd(com_run),
        ecolor='purple',
        capsize=4,
        fmt='none',
    )
    ...
    ax[1].errorbar(
        np.nanmean(e_H_run),
        np.nanmean(b_H_run),
        yerr=np.nanstd(b_H_run),
        xerr=np.nanstd(e_H_run),
        ecolor='seagreen',
        capsize=4,
        fmt='none',
    )
    ...
    ax[2].plot(
        np.nanmean(std_a_run),
        np.nanmean(std_M_run),
        marker='.',
        color='gray',
        markersize=15,
        markeredgecolor='black',
    ...
    ax[2].errorbar(
        np.nanmean(std_a_run),
        np.nanmean(std_M_run),
        yerr=np.nanstd(std_M_run),
        xerr=np.nanstd(std_a_run),
        ecolor='coral',
        capsize=4,
        fmt='none',
    )
